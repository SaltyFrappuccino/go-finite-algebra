...
func PrintVectorDemo(d linalg.Demo) {
	...
	fmt.Printf("Матрицы перехода базисов взаимно обратны: %s\n", boolText(d.BasisTransitionsInvert))
	fmt.Println()
	fmt.Println("Базис A:")
	...
	fmt.Printf("Декодирование координат обратно в v: %s\n", boolText(d.VectorDecoded))
	fmt.Println()
	fmt.Println("C(A<-B), матрица перехода базисов: столбцы B записаны в A:")
	fmt.Println(d.BasisTransitionAToB.Format())
	fmt.Println("C(B<-A), обратная матрица перехода базисов:")
	fmt.Println(d.BasisTransitionBToA.Format())
	fmt.Println("Матрица пересчета координат A->B, то есть C(A<-B)^-1:")
	fmt.Println(d.CoordinateTransitionAToB.Format())
	fmt.Printf("[v]_A = C[v]_B и [v]_B = C^-1[v]_A: %s\n", boolText(d.VectorContravariant))
	fmt.Println()
	...
	fmt.Printf("Ковектор w = %s как линейный функционал V -> %s\n", linalg.FormatVector(f, d.Covector), f.Name())
	fmt.Printf("[w]_A* = %s\n", linalg.FormatVector(f, d.CovectorCoordsA))
	fmt.Printf("[w]_B* = %s\n", linalg.FormatVector(f, d.CovectorCoordsB))
	fmt.Printf("[w]_B* = C^T[w]_A*: %s\n", boolText(d.CovectorCovariant))
	fmt.Printf("Значение w(v) не зависит от базиса: %s (%s)\n", boolText(d.CovectorEvaluationInvariant), f.ElementName(d.CovectorValueA))
	fmt.Println()
	fmt.Println("Изотропные ненулевые векторы для формы q(v)=sum xi^2 с единичной матрицей Грама:")
